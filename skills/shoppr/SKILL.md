...
- **Built-in throttle:** requests to `allegro.pl` are spaced ≥4 s (+0–2 s jitter) and `www.amazon.pl` ≥1.5 s, shared across *all* shoppr processes via a lock file in `~/.config/shoppr`. Bursts get the DataDome cookie flagged (a proxy doesn't help then — the block follows the cookie, and a fresh cookie needs DataDome's JS challenge). So batch jobs are slow by design; don't work around it with parallel agents. `SHOPPR_MIN_GAP_MS` overrides the gap (`0` disables). Ceneo and Allegro reviews (`edge.allegro.pl`) aren't throttled.
